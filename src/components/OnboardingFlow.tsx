        ...
        {step === 1 ? (
          ...
          <form onSubmit={handleFinalSubmit} className="space-y-3.5">
            <button
              type="button"
              onClick={onSwitchToLogin}
              className="w-full py-2 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-400 hover:bg-amber-500/20 text-[11px] font-bold flex items-center justify-center gap-1.5"
            >
              <UserCheck className="w-3.5 h-3.5" /> Already have an account? Log in here (no payment needed)
            </button>
            ...
            {registrationFeeEnabled && (
              <>
                ...
                >
                  <KeyRound className="w-3.5 h-3.5" /> Moderator / Sub-Moderator? Enter code to skip payment
                </button>
                {showBypass && (
                  <input
                    type="text"
