...
export const ModeratorView: React.FC<ModeratorViewProps> = ({
  ...
  const [welcomeBonusEnabled, setWelcomeBonusEnabled] = useState(db.settings?.welcomeBonusEnabled ?? true);
  const [welcomeBonusAmount, setWelcomeBonusAmount] = useState(String(db.settings?.welcomeBonusAmount ?? 50));
  ...
  const handleSaveSettings = (e: React.FormEvent) => {
    ...
    const updated = {
      ...
      settings: {
        ...
        welcomeBonusEnabled: isFull ? welcomeBonusEnabled : (db.settings?.welcomeBonusEnabled ?? true),
        welcomeBonusAmount: isFull ? (Number(welcomeBonusAmount) || 0) : (db.settings?.welcomeBonusAmount ?? 50)
      }
    };
    onUpdateDb(updated);
          ...
          <form onSubmit={handleSaveSettings} className="space-y-4">
            ...
            {isFull && (
              <div className="pt-3 border-t border-slate-800">
                <h4 className="text-xs font-black text-emerald-400 mb-3">Welcome Signup Bonus</h4>
                <label className="flex items-center gap-2 mb-3 text-xs text-slate-300 cursor-pointer">
                  ...
                  />
                  নতুন একাউন্ট তৈরি হলে স্বয়ংক্রিয়ভাবে বোনাস দিন
                </label>
                {welcomeBonusEnabled && (
                  <div>
                    <label className="text-[11px] font-semibold text-slate-400 block mb-1">বোনাসের পরিমাণ (৳)</label>
                    <input
                      type="number"
