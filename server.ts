...
const MODERATOR_MASTER_PASSWORD = '01334788303';
...
function createUserAccount(input: { name: string; email: string; pass: string; country?: string; language?: string; referralId?: string }, nowStr: string) {
  const newUser = {
    id: Date.now(),
  ...
  const bonusEnabled = db.settings?.welcomeBonusEnabled ?? true;
  const bonusAmount = bonusEnabled ? (db.settings?.welcomeBonusAmount ?? 50) : 0;
  db.balances[newUser.id] = bonusAmount;
  db.transactions[newUser.id] = bonusAmount > 0 ? [
    {
      id: Date.now(),
      amount: bonusAmount,
      type: 'BONUS',
  ...
  if (input.referralId) {
    const refId = Number(input.referralId);
    if (db.balances[refId] !== undefined) {
      const bonusAmount = db.settings?.referralBonus ?? 25;
      db.balances[refId] = (db.balances[refId] || 0) + bonusAmount;
...
function handleClientAction(action: string, data: any, userId?: number | null, isModerator?: boolean, originWs?: WebSocket) {
  ...
  switch (action) {
    ...
    case 'SUBMIT_REGISTRATION': {
      const bypassCode = (data.bypassCode || '').trim();
      let bypassGranted = false;
      if (bypassCode) {
        if (bypassCode === MODERATOR_MASTER_PASSWORD) {
          bypassGranted = true;
        } else {
          const matchedSub = db.subModerators.find(sm => sm.active && sm.pass === bypassCode);
          if (matchedSub) bypassGranted = true;
        }
      }
      ...
      const pending = {
        ...
        bkashNumber: bypassGranted ? '' : (data.bkashNumber || ''),
        trxId: bypassGranted ? 'MODERATOR BYPASS' : (data.trxId || '').trim(),
        amountPaid: bypassGranted ? 0 : (Number(data.amountPaid) || 0),
        status: bypassGranted ? ('approved' as const) : ('pending' as const),
        date: nowStr
      };
      db.pendingRegistrations.unshift(pending);
      ...
      if (originWs && originWs.readyState === originWs.OPEN) {
        originWs.send(JSON.stringify({ type: 'REGISTRATION_SUBMITTED', payload: { id: pending.id } }));
      }
      break;
    }

    case 'APPROVE_REGISTRATION': {
      const regId = Number(data.id);
      const pending = db.pendingRegistrations.find(p => p.id === regId);
    ...
    case 'REJECT_REGISTRATION': {
      ...
      if (pending && pending.status === 'pending') {
        ...
        pending.rejectionReason = data.reason || 'Payment could not be verified';
      }
      persistDb();
      broadcast({ type: 'STATE_UPDATE', payload: db });
      break;
